...
public class Rainfall {
    ...
    public static double mean(String town, String strng) {
        ...
        for (String row : splited) {
                tempCity = row.substring(0, row.indexOf(':'));

                if(tempCity.equalsIgnoreCase(town)){
                    city = tempCity;
                    row = row.replaceAll("[^0-9\\.]", "|").replaceAll("\\|+", " ").trim();
        ...
        for (String row : splited) {
            ...
            if(tempCity.equalsIgnoreCase(town)){
                ...
                for (String val : values) {
                    ...
                    meanResult += tempVal;
                    result += Math.pow(tempVal, 2);
                }

                meanResult /= values.length;
                meanResult *= meanResult;
                result = (result / values.length) - meanResult;
